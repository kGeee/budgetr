...
import { settledOnly } from "@/lib/pending";
  ...
  return db
    .all<{ category: string; icon: string | null; thisMonth: number; prevMonth: number }>(
      sql`SELECT cat.name AS category, cat.icon AS icon,
          ...
          FROM transactions t
          JOIN categories cat ON cat.id = ${effectiveCatId("t")}
          WHERE ${settledOnly()} AND t.amount > 0 AND cat."group" = 'spending'
...
export function getVendorCategoryBreakdown(vendorKey: string): CategorySpend[] {
  ...
  return db
    .all<{ categoryId: string | null; name: string | null; icon: string | null; total: number }>(sql`
      ...
      FROM transactions t
      LEFT JOIN categories cat ON cat.id = ${effectiveCatId("t")}
      WHERE ${settledOnly()} AND t.amount > 0 AND ${match}
      GROUP BY cat.id
      ORDER BY total DESC`)
    .map((r) => ({
      categoryId: r.categoryId,
...
/** Unreviewed transactions, newest first — the review inbox. Always excludes
 * pending regardless of the "count pending" preference: a pending row is still
 * going to be replaced by its posted twin, so categorizing it is wasted work. */
  ...
  const r = db.get<{
    ...
    SELECT cat.id AS id, cat.name AS name, cat.icon AS icon, cat."group" AS "group",
           ...
           COALESCE((SELECT SUM(CASE WHEN t.amount > 0 THEN t.amount ELSE 0 END)
                     FROM transactions t
                     WHERE ${settledOnly()} AND ${effectiveCatId("t")} = cat.id), 0) AS spent,
           COALESCE((SELECT SUM(CASE WHEN t.amount < 0 THEN -t.amount ELSE 0 END)
                     FROM transactions t
                     WHERE ${settledOnly()} AND ${effectiveCatId("t")} = cat.id), 0) AS received,
           COALESCE((SELECT COUNT(*) FROM transactions t
                     WHERE ${settledOnly()} AND ${effectiveCatId("t")} = cat.id), 0) AS count
    FROM categories cat
    WHERE cat.id = ${id}`);
  if (!r) return null;
  return {
...
/** Every settled transaction that rolls up to this category, newest first
 * (pending included when the "count pending" preference is on, so the list
 * reconciles with the category's spend figure). */
export function getCategoryTransactions(id: string, limit = 500): TransactionRow[] {
  return selectTransactions(sql`${settledOnly()} AND ${txnInCategory(id)}`, limit);
}

export function getTransactionsByDate(date: string): TransactionRow[] {
  return selectTransactions(sql`${settledOnly()} AND t.date = ${date}`, 500);
}
  ...
  return db
    .all<{ month: string; spent: number; received: number; count: number }>(sql`
      ...
      SELECT substr(sr.date, 1, 7) AS month,
             ...
             COUNT(*) AS count
      FROM spend_rows sr
      WHERE ${settledOnly("sr")} AND sr.category_id = ${id}
      GROUP BY month
      ORDER BY month DESC
      LIMIT ${months}`)
    .map((r) => ({
 ...
 * re-tagged as Rent counts here too. `txCount` is the number of expense
 * transactions that make up "Total spent" (income rows are excluded).
 *
 * Runs over spend_rows so a split transaction contributes its parts rather than
 * its parent — which is what keeps a bill you split with friends showing only
 * your own share here. txCount counts distinct transactions, not split rows, so
 * a three-way split still reads as one purchase.
 */
  ...
  const row = db.get<{ income: number; expenses: number; txCount: number }>(
    sql`WITH ${spendRowsCte}
        SELECT
          SUM(CASE WHEN sr.amount < 0 THEN -sr.amount ELSE 0 END) AS income,
          SUM(CASE WHEN sr.amount > 0 THEN sr.amount ELSE 0 END) AS expenses,
          COUNT(DISTINCT CASE WHEN sr.amount > 0 THEN sr.txn_id END) AS txCount
        FROM spend_rows sr
        LEFT JOIN categories cat ON cat.id = sr.category_id
        WHERE ${settledOnly("sr")}
          AND (cat."group" IS NULL OR cat."group" != 'transfer')
          AND sr.date >= ${start} AND sr.date <= ${end}`,
  );
